...
/// so the flagship Avalonia UI can be hosted inside a NON-Avalonia process (FL Studio) and reparented
/// into FL's native host form. This is the Avalonia analog of the plugin's WPF <c>UiHost</c>: Avalonia
...
/// <see cref="EnsureStarted"/> spins the thread + <c>SetupWithoutStarting</c> + a permanent
/// <see cref="Dispatcher.MainLoop"/> once and is idempotent. Plugin enable/disable just creates/closes
/// the <see cref="AvaloniaChatView"/> window on this thread — the thread keeps running so a later
/// re-enable can reuse it.</para>
///
...
public sealed class EmbeddedAvaloniaHost
{
    ...
    /// <summary>True once Avalonia is set up and the dispatcher is pumping on our STA thread.</summary>
    public bool IsReady => _ready.IsSet && _startError is null;
    ...
    /// libSkiaSharp, which the caller catches to fall back to the WPF UI).
    /// </summary>
    ...
    private void ThreadMain()
    {
        ...
        // Permanent message loop for the process lifetime. Never cancelled: closing the chat window
