...
public sealed class ClipTools(INativeFlControl fl)
{
    ...
    [Description("List playlist clips (slot index, track, start, length, source pattern/channel, mute), paged.")]
    public Task<string> ListClips(
        [Description("Skip first N matching clips (the result's continuation hint gives the next offset)")] int offset = 0,
        [Description("Playlist track filter (1-based), or -1 = all")] int track = -1, CancellationToken ct = default) =>
        McpSupport.SafeAsync(() => fl.ListClipsAsync(offset, track, ct));
