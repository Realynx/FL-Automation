...
public sealed class PluginManager : IPluginManager, IDisposable
{
    ...
    private async Task ActivateAsync_NoLock(PluginEntry entry, CancellationToken ct)
    {
        ...
        // One context per plugin so its Menu + Toolbar contributions are tagged with this plugin's id
        // (and so removed as a set on disable/unload). Cheap to build; the registrars just close over the id.
        IPluginContext context = new PluginContext(_fl, _services, _log, _menu.ScopeFor(entry.Id), _toolbar.ScopeFor(entry.Id));
