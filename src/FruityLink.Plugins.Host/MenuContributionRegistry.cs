...
/// native caller or take the host down.</para>
/// </summary>
public sealed class MenuContributionRegistry : IFlMenuContributions
{
    ...
    public MenuContributionRegistry(Action<string>? log = null) => _log = log;
    ...
    {
        try { Changed?.Invoke(); }
        catch (Exception ex) { _log?.Invoke("menu-registry: Changed handler threw: " + ex.Message); }
    }
    ...
    public string ListJson()
    {
        ...
        for (int i = 0; i < snapshot.Length; i++)
        {
            ...
            sb.Append("{\"id\":\"").Append(Esc(c.Id))
              ...
              .Append("\",\"checked\":").Append(EvalChecked(c) ? "true" : "false")
              .Append('}');
        }
        sb.Append(']');
        return sb.ToString();
    ...
    public bool Invoke(string id)
    {
        ...
        catch (Exception ex) { _log?.Invoke($"menu-registry: handler for '{id}' threw: {ex.Message}"); return false; }
    }
    ...
    public int Checked(string id)
    {
        ...
        return EvalChecked(found) ? 1 : 0;
    }

    // Evaluate a toggle's isChecked OUTSIDE the lock, exception-guarded. Commands report false.
    private bool EvalChecked(Contribution c)
    {
        if (!c.IsToggle || c.IsChecked is null) return false;
        try { return c.IsChecked(); }
        catch (Exception ex) { _log?.Invoke($"menu-registry: isChecked for '{c.Id}' threw: {ex.Message}"); return false; }
    }

    // Minimal JSON string escaping: only the two characters the native unescaper handles, plus
    // newlines/tabs flattened to spaces so a caption never breaks the single-line menu text.
    ...
    {
        private MenuContributionRegistry? _owner;
        private readonly string _id;
        public Handle(MenuContributionRegistry owner, string id) { _owner = owner; _id = id; }
        public void Dispose()
        {
            MenuContributionRegistry? o = System.Threading.Interlocked.Exchange(ref _owner, null);
            o?.Remove(_id);
        }
    }
}
