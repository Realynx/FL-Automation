    ...
    string Model = AccountSettings.DefaultModel)
{
    ...
    /// <summary>
    /// Max auto-invoke tool rounds (model round-trips, each possibly batching several tool calls) a
    /// single turn may run. <b>Default 0 = UNLIMITED</b> (0 or any negative value): a big job legitimately
    /// chains many rounds, and the artificial cap was cutting honest work short — the user's STOP button
    /// is the control for a genuinely-wrong turn, with SK's internal auto-invoke limit + "say continue"
    /// as soft backstops. A positive value re-imposes a cap (clamped to [1, 200] at the use site). A manual
    ...
    /// <summary>
    /// Opt-in remote debugging: upload each agent turn's debug transcript (tool calls with args +
    /// results, LLM responses, token usage, timings — the same record <c>turns-*.jsonl</c> keeps
    /// locally) to the FL Automate gateway so the team can review beta sessions remotely and improve
    /// the tool service. <b>Default FALSE — nothing leaves the machine unless the user turns on
    /// "Share debug data" in Settings.</b> Kept OUT of the positional parameter list so every
    /// existing construction site compiles unchanged.
    /// </summary>
    public bool ShareDebugData { get; init; }
