        ...
        var t = new Thread(() =>
        {
            for (int i = 0; i < 30; i++)
            {
                try
                {
                    ...
                    try { InProcBridge.Raw("toolbar_button_refresh"); } catch { /* best-effort */ }
    ...
    private static void InitializeRealPluginHost()
    {
        try
        {
            ...
            if (mgr is FruityLink.Plugins.Host.PluginManager pm)
            {
                ...
                pm.ToolbarRegistry.Changed += () =>
                {
                    try { InProcBridge.Raw("toolbar_button_refresh"); }
                    catch (Exception ex) { Log("toolbar_button_refresh failed: " + ex.Message); }
                };
            }
            ...
            try { Log("toolbar_button_refresh (post-init) -> " + InProcBridge.Raw("toolbar_button_refresh")); }
            catch (Exception ex) { Log("toolbar_button_refresh (post-init) failed: " + ex.Message); }
        }
