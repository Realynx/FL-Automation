...
The framework installer supplies the private CPython runtime and Python SDK wheel. No system Python, pip, or virtual environment is required. Shared Avalonia and the initial exclusive native window host shipped with installer **0.1.13**. The native geometry correction is prepared for **0.1.15** after the initial 0.1.14 implementation fell back to external windows; see [the consumer hosting contract](../../docs/plugin-window-consumers.md). The earlier live IDE checks covered FL Studio 2026. The new simultaneous-frame behavior still needs live checks in FL Studio 2025 and 2026; source and headless tests are not a substitute for that validation. Keep the full publish directory together, including native Skia/HarfBuzz assets. Do not copy only the plugin DLL.
...
The archive goes to `sdk/artifacts/python-ide`; it does not install or enable anything. See `THIRD-PARTY-NOTICES.md` for the included toolkit licenses.
