from typing import Any

import pytest
from conftest import RecordingTransport

from fruitylink import ProtocolError, Studio
from fruitylink.levels import FADER_MAX_DB, mixer_volume_from_db
from fruitylink.mixer import MAX_INSERTS, MixerTrack
from fruitylink.values import JsonValue


def operations(transport: RecordingTransport) -> list[tuple[str, dict[str, Any]]]:
    result: list[tuple[str, dict[str, Any]]] = []
    for _, params in transport.calls:
        arguments = params["arguments"]
        assert isinstance(arguments, dict)
        result.append((str(params["operation"]), dict(arguments)))
    return result


def test_add_appends_without_renaming_or_routing(fl: Studio, transport: RecordingTransport) -> None:
    transport.responses["add_mixer_track"] = 126

    track = fl.mixer.add()

    assert isinstance(track, MixerTrack)
    assert track.index == 126
    assert transport.calls == [("invoke", {"operation": "add_mixer_track", "arguments": {"afterTrack": -1}})]


@pytest.mark.parametrize("after,new_index", [(0, 1), (3, 4), (125, 126)])
def test_add_names_only_returned_insert(fl: Studio, transport: RecordingTransport,
                                      after: int, new_index: int) -> None:
    transport.responses["add_mixer_track"] = new_index
    name = "Bus 🎛 — alternate"

    track = fl.mixer.add(name, after=after)

    assert track.index == new_index
    assert transport.calls == [
        ("invoke", {"operation": "add_mixer_track", "arguments": {"afterTrack": after}}),
        ("invoke", {"operation": "set_mixer_track_name", "arguments": {"track": new_index, "name": name}}),
    ]


@pytest.mark.parametrize("after", [-1, -2, True, False, 1.5, "3"])
def test_add_rejects_invalid_explicit_after_before_mutation(fl: Studio, transport: RecordingTransport,
                                                          after: Any) -> None:
    with pytest.raises(IndexError):
        fl.mixer.add(after=after)
    assert transport.calls == []


@pytest.mark.parametrize("name", [True, 42, [], {}])
def test_add_rejects_invalid_name_before_mutation(fl: Studio, transport: RecordingTransport,
                                                name: Any) -> None:
    with pytest.raises(TypeError, match="name"):
        fl.mixer.add(name)
    assert transport.calls == []


@pytest.mark.parametrize("index", [0, -1, True, None, "4"])
def test_invalid_creation_reply_never_renames_existing_track(fl: Studio, transport: RecordingTransport,
                                                           index: JsonValue) -> None:
    transport.responses["add_mixer_track"] = index
    with pytest.raises(ProtocolError, match="inspect mixer state"):
        fl.mixer.add("New insert")
    assert len(transport.calls) == 1


def test_rename_failure_does_not_retry_structural_edit(fl: Studio, transport: RecordingTransport) -> None:
    def handler(method: str, arguments: dict[str, JsonValue]) -> JsonValue:
        if arguments["operation"] == "add_mixer_track":
            return 4
        raise RuntimeError("Rename failed after insert was created")

    transport.handler = handler
    with pytest.raises(RuntimeError, match="Rename failed"):
        fl.mixer.add("New insert", after=3)
    assert [arguments["operation"] for _, arguments in transport.calls] == [
        "add_mixer_track", "set_mixer_track_name"]


def test_raw_operation_preserves_native_default_and_return_value(fl: Studio,
                                                               transport: RecordingTransport) -> None:
    transport.responses["add_mixer_track"] = 126
    assert fl.ops.add_mixer_track() == 126
    assert transport.calls == [("invoke", {"operation": "add_mixer_track", "arguments": {"afterTrack": -1}})]


def test_mixer_iteration_uses_typed_indices_without_inventing_current(fl: Studio,
                                                                  transport: RecordingTransport) -> None:
    transport.responses["get_mixer_track_count"] = 18
    transport.responses["query_mixer_tracks"] = [
        {"index": 0, "name": "Master", "kind": "master"},
        {"index": 3, "name": "Piano\nBus", "kind": "insert"},
        {"index": 16, "name": "Last ordinary", "kind": "insert"},
    ]

    assert [(item.index, item.kind) for item in fl.mixer.list()] == [(0, "master"), (3, "insert"), (16, "insert")]
    assert len(fl.mixer) == 3
    assert [track.index for track in fl.mixer] == [0, 3, 16]
    assert fl.mixer.find("Piano\nBus").index == 3
    assert all(arguments["operation"] == "query_mixer_tracks" for _, arguments in transport.calls)


def test_mixer_find_refuses_duplicate_names_in_typed_snapshot(fl: Studio,
                                                           transport: RecordingTransport) -> None:
    transport.responses["query_mixer_tracks"] = [
        {"index": 3, "name": "Bus", "kind": "insert"},
        {"index": 4, "name": "Bus", "kind": "insert"},
    ]
    with pytest.raises(LookupError, match="found 2"):
        fl.mixer.find("Bus")
    assert len(transport.calls) == 1


def test_insert_count_and_capacity_come_from_the_native_count(fl: Studio, transport: RecordingTransport) -> None:
    transport.responses["get_mixer_track_count"] = 18
    assert fl.mixer.insert_count == 16
    assert fl.mixer.capacity == MAX_INSERTS == 500
    transport.responses["get_mixer_track_count"] = 2
    with pytest.raises(ProtocolError):
        fl.mixer.insert_count


def test_ensure_inserts_appends_until_the_count_is_reached(fl: Studio, transport: RecordingTransport) -> None:
    inserts = 16

    def handler(method: str, arguments: dict[str, JsonValue]) -> JsonValue:
        nonlocal inserts
        if arguments["operation"] == "get_mixer_track_count":
            return inserts + 2
        if arguments["operation"] == "add_mixer_track":
            assert arguments["arguments"] == {"afterTrack": -1}
            inserts += 1
            return inserts
        raise AssertionError(arguments)

    transport.handler = handler
    assert fl.mixer.ensure_inserts(18) == 2
    assert inserts == 18
    assert fl.mixer.ensure_inserts(18) == 0
    assert fl.mixer.ensure_inserts(4) == 0
    assert [op for op, _ in operations(transport)].count("add_mixer_track") == 2


@pytest.mark.parametrize("count", [501, -1, 2.0, True])
def test_ensure_inserts_refuses_impossible_counts_before_any_edit(fl: Studio, transport: RecordingTransport,
                                                                count: Any) -> None:
    with pytest.raises(IndexError):
        fl.mixer.ensure_inserts(count)
    assert transport.calls == []


def test_send_to_supports_unity_db_and_disconnect(fl: Studio, transport: RecordingTransport) -> None:
    track = fl.mixer[8]
    track.send_to(15, 0.8)
    track.send_to(0, 0.0)
    track.send_to(15, db=0.0)
    track.send_to(15)
    track.send_to(0, 0.0, active=False)
    track.disconnect(0)
    sent = operations(transport)
    assert [op for op, _ in sent] == ["set_mixer_send"] * 6
    assert [args for _, args in sent] == [
        {"srcTrack": 8, "dstTrack": 15, "level": 0.8, "active": True},
        {"srcTrack": 8, "dstTrack": 0, "level": 0.0, "active": True},
        {"srcTrack": 8, "dstTrack": 15, "level": pytest.approx(0.8), "active": True},
        {"srcTrack": 8, "dstTrack": 15, "level": 1.0, "active": True},
        {"srcTrack": 8, "dstTrack": 0, "level": 0.0, "active": False},
        {"srcTrack": 8, "dstTrack": 0, "level": 0.0, "active": False},
    ]
    with pytest.raises(TypeError):
        track.send_to(15, 0.8, db=0.0)


def test_sends_and_routes_are_typed_readback(fl: Studio, transport: RecordingTransport) -> None:
    sends: dict[int, JsonValue] = {
        0: [],
        8: [{"source": 8, "destination": 0, "destinationName": "Master", "level": 0.0, "active": True},
            {"source": 8, "destination": 15, "destinationName": "Drum Bus", "level": 0.8, "active": True}],
        15: [{"source": 15, "destination": 0, "destinationName": "Master", "level": 1.0, "active": True}],
    }
    tracks: list[JsonValue] = [{"index": index, "name": f"T{index}", "kind": "master" if index == 0 else "insert"}
                               for index in sends]

    def handler(method: str, arguments: dict[str, JsonValue]) -> JsonValue:
        if arguments["operation"] == "query_mixer_tracks":
            return tracks
        if arguments["operation"] == "query_mixer_sends":
            wire = arguments["arguments"]
            assert isinstance(wire, dict) and isinstance(wire["track"], int)
            return sends[wire["track"]]
        raise AssertionError(arguments)

    transport.handler = handler
    drum = fl.mixer[8].sends()
    assert [(s.destination, s.destination_name, s.level, s.active) for s in drum] == [
        (0, "Master", 0.0, True), (15, "Drum Bus", 0.8, True)]
    assert drum[1].level_db == pytest.approx(0.0)
    assert drum[0].level_db == float("-inf")
    assert fl.mixer[8].send_level(15) == 0.8
    assert fl.mixer[8].send_level(0) == 0.0
    assert fl.mixer[8].send_level(3) is None
    routes = fl.mixer.routes()
    assert [(r.source, r.destination, r.level) for r in routes] == [(8, 0, 0.0), (8, 15, 0.8), (15, 0, 1.0)]
    assert routes[2].level_db == pytest.approx(FADER_MAX_DB)   # level 1.0 = the knob top


def test_volume_db_uses_the_mixer_scale(fl: Studio, transport: RecordingTransport) -> None:
    transport.responses["get_mixer_volume"] = 12800
    assert fl.mixer[2].volume_db == pytest.approx(0.0)
    fl.mixer[2].volume_db = FADER_MAX_DB
    minus_three = mixer_volume_from_db(-3.0)
    assert 10800 < minus_three < 10900   # calibrated curve (exponent 2.09), not the old 11358
    assert fl.mixer.master.set_volume(db=-3.0) == minus_three
    assert fl.mixer[2].set_volume(16000) == 16000
    assert [args for _, args in operations(transport)][1:] == [
        {"track": 2, "value": 16000}, {"track": 0, "value": minus_three}, {"track": 2, "value": 16000}]
    with pytest.raises(IndexError):
        fl.mixer[2].set_volume(16001)
    with pytest.raises(TypeError):
        fl.mixer[2].set_volume()
