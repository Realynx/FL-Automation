...
public sealed class FlAgentPlugin : IFlPlugin
{
        ...
        FruityLink.Agent.FlAgent agent, IDictationService? dictation, bool ownsDictation, bool wantEmbed, IPluginContext context)
    {
        ...
        // Settings "AI CONNECTION" card → the agent's real settings/secret stores + live re-configure.
        var backendSettings = AgentComposition.BuildBackendGateway(agent);
        var presenter = new AvaloniaChatPresenter(host, chat.ViewModel, agent, context.Log, dictation, ownsDictation, backendSettings);
