...
internal static class AgentComposition
{
    ...
    // location as before (%APPDATA%\FruityLink → settings.json / secrets.json), so these instances
    ...
    public static FruityLink.Agent.FlAgent ResolveOrBuild(IPluginContext context)
    {
        ...
        var secretStore = SharedSecrets;
        var diagnostics = new LlmDiagnostics(Path.Combine(SharedPaths.BaseDirectory, "logs"));
        var kernelFactory = new ChatKernelFactory(diagnostics);
        ...
        var subAgents = new SubAgentService(
            kernelFactory, settingsStore, secretStore, music, nativeControl, knowledge, toolFilter);
        var orchestration = new OrchestrationPlugin(subAgents, settingsStore);
        var pluginSet = new FlPluginSet(music, nativeControl, knowledge, orchestration);

        return new FruityLink.Agent.FlAgent(kernelFactory, settingsStore, secretStore, pluginSet, toolFilter);
    }

    /// <summary>
    /// Builds the gateway the Avalonia Settings "AI CONNECTION" card uses to read/write the agent's
    /// backend config. It targets the SAME on-disk stores the agent loads from (<c>%APPDATA%\FruityLink</c>
    /// via <see cref="StoragePaths"/>): config → <c>settings.json</c>, API key → encrypted
    /// <c>secrets.json</c>. On save it re-configures <paramref name="agent"/> live. Because
    /// <see cref="StoragePaths"/> is deterministic, these store instances address the exact same files the
    /// agent uses whether it's the host's shared agent or a self-composed one.
    /// </summary>
    public static IBackendSettingsGateway BuildBackendGateway(FruityLink.Agent.FlAgent agent)
    {
        return new AgentBackendSettingsGateway(SharedSettings, SharedSecrets, agent);
    }
