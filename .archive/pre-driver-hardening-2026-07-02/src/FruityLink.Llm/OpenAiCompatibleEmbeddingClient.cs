...
public sealed class OpenAiCompatibleEmbeddingClient : IEmbeddingClient
{
    ...
    /// <summary>
    /// Creates a client for the resolved embeddings connection.
    /// </summary>
    /// <param name="http">Injected HTTP client. Not disposed by this type.</param>
    /// <param name="settings">
    /// Resolved connection (endpoint + model). Use <see cref="EmbeddingSettings.Resolve"/> to obtain
    /// the effective <see cref="LlmSettings"/> with the embedding model already applied.
    /// </param>
    /// <param name="apiKey">
    ...
    /// </param>
    public OpenAiCompatibleEmbeddingClient(HttpClient http, LlmSettings settings, string? apiKey = null)
    {
