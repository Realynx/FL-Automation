...
public sealed class PluginManager : IPluginManager, IDisposable
{
    ...
    public PluginManager(INativeFlControl fl, IServiceProvider services, string pluginsDir, string stateFile, string logFile, string shadowRoot)
    {
        ...
        _shadowRoot = shadowRoot ?? throw new ArgumentNullException(nameof(shadowRoot));
        _log = WriteLog;
        _fl = fl;
        _services = services;
    ...
    {
        HashSet<string> enabledIds = LoadPersistedEnabled();
        int before;
    ...
    private IEnumerable<string> EnumerateCandidateDlls()
    {
        ...
        foreach (string f in SafeEnumerate(_pluginsDir, SearchOption.TopDirectoryOnly))
            if (IsPluginCandidate(f) && seen.Add(f))
                yield return f;
    ...
    private void DiscoverDll(string dllPath, HashSet<string> enabledIds)
    {
        ...
        try
        {
            (shadowDir, string shadowDll) = ShadowCopy(dllPath);
    ...
    private async Task<bool> ReloadDll_NoLock(string originalDllPath, CancellationToken ct)
    {
        ...
        lock (_sync) affected = _plugins.Values.Where(e => PathEquals(e.Owner.Path, originalDllPath)).ToList();
        ...
        lock (_sync) fresh = _plugins.Values.Where(e => PathEquals(e.Owner.Path, originalDllPath)).ToList();
    ...
    {
        HashSet<string> persisted = LoadPersistedEnabled();
    ...
    {
        List<PluginEntry> fresh;
        lock (_sync) fresh = _plugins.Values.Where(e => PathEquals(e.Owner.Path, path) && e.EnabledDesired && !e.Active).ToList();
    ...
    private async Task ActivateAsync_NoLock(PluginEntry entry, CancellationToken ct)
    {
        ...
        // One context per plugin so its Menu contributions are tagged with this plugin's id (and so
        // removed as a set on disable/unload). Cheap to build; the registrar just closes over the id.
        IPluginContext context = new PluginContext(_fl, _services, _log, _menu.ScopeFor(entry.Id));
    ...
    private async Task<bool> DeactivateAsync_NoLock(PluginEntry entry, CancellationToken ct)
    {
        ...
        // Belt-and-suspenders: drop ALL of this plugin's menu contributions even if its DisableAsync
        // forgot to dispose them (or threw). Covers disable, reload, and shutdown-deactivate uniformly.
        _menu.RemoveByPlugin(entry.Id);
        lock (_sync) { entry.Active = false; entry.Instance = null; } // drop instance ref so the ALC can unload
        return ok;
    }
    ...
    private IFlPlugin EnsureLoaded_NoLock(PluginEntry entry)
    {
        ...
        if (asm is null)
        {
            (string shadowDir, string shadowDll) = ShadowCopy(owner.Path);
    ...
    private void UnloadAlc(PluginAssembly owner)
    {
        ...
        if (shadow is not null) TryDeleteDir(shadow);
    }
    ...
    private (string dir, string dll) ShadowCopy(string originalDll)
    {
    ...
    private HashSet<string> LoadPersistedEnabled()
    {
        try
        {
